...
'''
i think it should be parsed with a decorator for each view that might handle these kinds of url
'''
...
class DummyParsedRequest:
    url: str
    isForeignRequest: bool
    parsedId: str #could be id or a link?
    

def viewWrapper(func):
    
    def inner(request : DummyRequest, *args, **kwargs):
        
        incomingRequest = request
        parsed = f"parsed! {request.url}" # lets pretend we parsed this url with some magic
        isForeign = True
        
        parsedRequest = DummyParsedRequest(url=request.url, isForeignRequest=isForeign, parsedId=parsed)
        func(parsedRequest)
...
def someView(request):
    ...
    print(request.url)
    if request.isForeignRequest:
        print(f"sending request! to {request.parsedId}")

someView(DummyRequest("requesting url"))